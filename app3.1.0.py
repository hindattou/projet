...
class Contact:
    def __init__(self, nom,prénom, email, telephone):
        ...
        if not isinstance(prénom, str) or not nom.strip():
...
class CarnetAdresses:
    ...
    def sauvegarder_contacts(self):
        with open("contactsfile.txt", "w") as f:
            f.write('nom,prenom,email,telephone\n')
            for contact in self.contacts.values():
                f.write(contact.nom + ","+ contact.prénom +"," + contact.email + "," + contact.telephone + "\n")
    ...
    def charger_contacts(self):
        try:
            with open("contactsfile.txt", "r") as f:
                for ligne in f:
                    ...
                    if len(parts) == 4:
                        ...
                        try:
                            ...
                            messagebox.showerror("Erreur", f"Erreur lors du chargement du contact {nom} : données invalides.")
...
class App:
    ...
    def rechercher_contact(self):
        ...
        entry_nom.pack()
        entry_prénom = tk.Entry(fenetre_recherche)
        entry_prénom.pack()
    ...
    def supprimer_contact(self):
        ...
        tk.Label(fenetre_suppr, text="contact à supprimer :").pack()
