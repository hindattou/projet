...
class CarnetAdresses:
    ...
    def rechercher_contact(self, valeur):
        ...
        if not valeur:
            return "Veuillez entrer un nom ou prénom valide."
        matches = []
        for contact in self.contacts.values():
            if contact.nom.lower() == valeur or contact.prénom.lower() == valeur:
                matches.append(contact.afficher())
        if not matches:
    ...
    def charger_contacts(self):
        try:
            with open("contacts.csv", "r") as f:
                lines = f.readlines()
                if lines and lines[0].strip() == 'nom,prenom,email,telephone':  # Skip header
                    lines = lines[1:]
                for ligne in lines:
...
class App:
    ...
    def rechercher_contact(self):
        ...
        tk.Label(fenetre_recherche, text="Nom ou Prénom à rechercher :").pack()
        entry_valeur = tk.Entry(fenetre_recherche)
        entry_valeur.pack()

        def rechercher():
            valeur = entry_valeur.get()
            result = self.carnet.rechercher_contact(valeur)
            self.text_affichage.delete(1.0, tk.END)
    ...
    def supprimer_contact(self):
        ...
        tk.Label(fenetre_suppr, text="Nom ou Prénom du contact à supprimer :").pack()
        entry_valeur = tk.Entry(fenetre_suppr)
        entry_valeur.pack()

        def supprimer():
            valeur = entry_valeur.get()
            result = self.carnet.supprimer_contact(valeur)
            if result == "multiple":
                messagebox.showerror("Erreur", "Plusieurs contacts trouvés avec ce prénom. Veuillez utiliser le nom exact.")
            elif result:
